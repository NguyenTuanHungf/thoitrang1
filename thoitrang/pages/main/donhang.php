...
if (isset($_SESSION['email'])) {
    ...
    $totalOrdersQuery = mysqli_query($mysqli, "SELECT COUNT(*) as total FROM tbl_giohang, tbl_khackhang WHERE tbl_giohang.id_khachhang = tbl_khackhang.id_khachhang AND tbl_khackhang.email = '$userEmail' AND ('$category' = 'all' OR tbl_giohang.cart_status = '$category')");
    ...
    AND ('$category' = 'all' OR tbl_giohang.cart_status = '$category')
    ORDER BY tbl_giohang.stime DESC
    LIMIT $begin, $ordersPerPage";


    $query_lietke_dh = mysqli_query($mysqli, $sql_lietke_dh);
// Đếm số đơn hàng cho mỗi trạng thái
$countPending = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT COUNT(*) as count FROM tbl_giohang WHERE cart_status = 0 AND id_khachhang IN (SELECT id_khachhang FROM tbl_khackhang WHERE email = '$userEmail')"))['count'];
$countWaiting = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT COUNT(*) as count FROM tbl_giohang WHERE cart_status = 1 AND id_khachhang IN (SELECT id_khachhang FROM tbl_khackhang WHERE email = '$userEmail')"))['count'];
$countDelivering = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT COUNT(*) as count FROM tbl_giohang WHERE cart_status = 2 AND id_khachhang IN (SELECT id_khachhang FROM tbl_khackhang WHERE email = '$userEmail')"))['count'];
$huy = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT COUNT(*) as count FROM tbl_giohang WHERE cart_status = 4 AND id_khachhang IN (SELECT id_khachhang FROM tbl_khackhang WHERE email = '$userEmail')"))['count'];
$hoantra = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT COUNT(*) as count FROM tbl_hoanhang WHERE status_lh = 1 AND id_khachhang IN (SELECT id_khachhang FROM tbl_khackhang WHERE email = '$userEmail')"))['count'];
...
<tbody>
    ...
    while ($row = mysqli_fetch_array($query_lietke_dh)) {
        ...
        $deliveryDate = $row['stime']; // Ngày giao hàng từ CSDL
        ...
        if ($cartStatus == 2 && strtotime($currentDate) > strtotime($deliveryDate . ' +7 days')) {
            ...
            $sql_update_status = "UPDATE tbl_giohang SET cart_status = 3 WHERE code_cart = '" . $row['code_cart'] . "'";
    ...
    <div class="order-info">
        ...
        <div class="product-price"> <?php echo ($row['stime']) ; ?></div>
...
<div class="pagination">
 ...
 for ($i = 1; $i <= $totalPages; $i++) {
    ...
    echo "Không có đơn hàng nào"; // Nếu không tồn tại $_SESSION['email'], hiển thị thông báo
}
    ?>
</div>
